...
PROMPTS = {
    ...
    "deep_context": [
        "Given this code:\n```python\ndef fibonacci(n):\n    if n <= 1: return n\n    a, b = 0, 1\n    for _ in range(2, n+1):\n        a, b = b, a+b\n    return b\n\ndef is_prime(n):\n    if n < 2: return False\n    for i in range(2, int(n**0.5)+1):\n        if n % i == 0: return False\n    return True\n\ndef prime_fibs(limit):\n    return [fibonacci(i) for i in range(limit) if is_prime(fibonacci(i))]\n```\nExplain what prime_fibs does, analyze its time complexity, and suggest optimizations.",
        "Here is a passage: 'Machine learning is a subset of artificial intelligence that focuses on building systems that learn from data. Unlike traditional programming where rules are explicitly coded, ML systems identify patterns in data to make decisions. There are three main types: supervised learning (labeled data), unsupervised learning (unlabeled data), and reinforcement learning (reward-based). Deep learning, a subset of ML, uses neural networks with many layers. Key challenges include overfitting, underfitting, data quality, and computational cost.' Summarize this in exactly 3 bullet points.",
    ],
